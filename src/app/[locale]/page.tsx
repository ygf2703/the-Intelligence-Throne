    ...
    <main className="site-page" dir={he ? "rtl" : "ltr"}>
      ...
      <section className="hero">
        ...
        <div className="hero-grid">
          <div>
            <span className="eyebrow">{he ? "רומן בהמשכים על תבונה וסמכות" : "A SERIAL NOVEL ABOUT INTELLIGENCE & AUTHORITY"}</span>
            ...
            <div className="hero-actions"><Link className="button primary" href={`/${locale}/read/beyond-the-threshold`}>{he ? "התחל בפרולוג" : "Read the prologue"}</Link><Link className="button secondary" href={`/${locale}/book`}>{he ? "למדף הספר" : "Browse the book"}</Link></div>
          ...
          <aside className="release-panel" aria-label={he ? "Astra — האדריכל הראשון" : "Astra — The First Architect"}>
            <div className="release-art release-character-art"><Image src="/assets/characters/astra/portrait-cast-v2.png" alt="Astra" fill priority sizes="(max-width: 1050px) 100vw, 34rem" /><span>THE APERTURE DOMINION</span><em>{he ? "Astra · האדריכל הראשון" : "Astra · The First Architect"}</em></div>
      ...
      <div className="manifesto-band"><span>{he ? "אנשים" : "PEOPLE"}</span><i /><span>{he ? "רעיונות" : "IDEAS"}</span><i /><span>{he ? "מערכות" : "SYSTEMS"}</span><i /><span>{he ? "עולמות" : "WORLDS"}</span></div>
